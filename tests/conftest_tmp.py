"""
tests/conftest_tmp.py - Pytest plugin module providing fixtures (workaround for conftest import issue)
"""
...
# Ensure project root on path for local imports in tests
...
def test_app():
    ...
    def key_stats():
        ...
        if not (request.headers.get("X-API-Key") or request.args.get("api_key")):
            return jsonify({"error": "API key required", "code": "AUTH_REQUIRED"}), 401
        return (
            jsonify({"client_name": "Test Client", "daily_usage": 0, "daily_limit": 1000, "remaining_today": 1000}),
            200,
        )
